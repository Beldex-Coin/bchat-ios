...
class NewBlockedContactVC: BaseVC, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        getBlockedContacts()
        updateRighBarButton(isSelectionEnable: isSelectionEnable)
    ...
    func updateRighBarButton(isSelectionEnable : Bool) {
        ...
        bottomButtonView.isHidden = !isSelectionEnable
    ...
    func getBlockedContacts() {
        ...
        for publicKey in contacts {
            ...
            if blockedflag == true {
                let userName = Storage.shared.getContact(with: publicKey)?.displayName(for: .regular)
    ...
    @objc private func unblockButtonTapped() {
        if self.selectedarrayPublicKey.count > 0 {
            ...
            getBlockedContacts()
            updateRighBarButton(isSelectionEnable: false)
