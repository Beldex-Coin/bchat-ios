...
class QRCodeScanningViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    public weak var scanDelegate: (UIViewController & QRScannerDelegate)?
    ...
    private lazy var maskLayer: CAShapeLayer = {
        ...
        result.fillColor = UIColor.beldexDarkestGray().cgColor
        result.opacity = 0.32
            ...
            if self.capture == nil {
                self.captureQueue.async { [weak self] in
                    ...
                    guard
                        ...
                        return Logger.info("Failed to retrieve the device for enabling the QRCode scanning camera")
                    }
                    ...
                    guard !capture.inputs.isEmpty && capture.outputs.count == 2 else {
                        return Logger.info("Failed to attach the input/output to the capture session")
                    }
                    
                    guard metadataOutput.availableMetadataObjectTypes.contains(.qr) else {
                        return Logger.info("The output is unable to process QR codes")
                    }
